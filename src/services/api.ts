...
export async function saveAndDownload(
    ...
    // Extraer el ID desde cualquier header x-*-id retornado por el router_factory
    const idHeaderKey = Object.keys(response.headers).find((k) => k.toLowerCase().endsWith('-id'))
    const parsedId = idHeaderKey ? Number(response.headers[idHeaderKey]) : NaN
    return {
        blob: response.data,
